...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    const ROLE_INTERPRETER = 'interpreter';
     ...
     */
    public function afterSignup(array $profileData = [],$role = null, $no_check_forum = false)
    {
        $this->refresh();
        ...
        $this->save();
        $auth->assign($auth->getRole($role ? $role : User::ROLE_USER), $this->getId());
        if(!(Yii::$app instanceof \yii\console\Application)) {
            if(!$no_check_forum) $this->checkIssetForumAccount();
        }
    }
